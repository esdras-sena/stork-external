...
const (
	...
	// Empty means the node estimates the tip, which is the production default.
	devnetTip = ""
...
func TestIntegrationPushAndPull(t *testing.T) {
	...
	interactor, err := NewContractInteractor(contractAddress, config.AccountAddress, "", devnetTip, keyFile, logger)
	...
	var updates map[types.InternalEncodedAssetID]types.AggregatedSignedPrice
	...
	require.NotNil(t, updates, "no fixture batch was fresh enough to apply")
	...
	pulled, err := interactor.PullValues(ctx, ids)
	require.NoError(t, err)
...
func TestIntegrationRepushedBatchIsRejected(t *testing.T) {
	...
	interactor, err := NewContractInteractor(
		contractAddress, config.AccountAddress, "", devnetTip, []byte(config.PrivateKey), logger,
	)
	require.NoError(t, err)
	require.NoError(t, interactor.ConnectHTTP(ctx, config.RpcUrl))
	...
	var applied map[types.InternalEncodedAssetID]types.AggregatedSignedPrice
	...
	require.NotNil(t, applied, "no fixture was fresh enough to apply")
